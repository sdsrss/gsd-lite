...
- [P1] [未复现] 自动更新的回滚路径在唯一会自动安装的模式（manual）下是死代码（两个独立原因）：
  ...
  主线程尚未独立复现，**R4 首选核验对象**。第二个原因：备份写在
  `~/.claude/gsd/package.json.bak`，而 `install.js` 会把整个 `~/.claude/gsd` 删掉，
  所以备份正好毁于它要防的那个失败窗口。后果（据 scout-hooks 阅读推断，未执行验证）：
  半装状态下 `getCurrentVersion()` 退化成 `'0.0.0'`，于是每天重试同一次失败的安装。
...
- [P1] [未复现] SessionStart 在 composite statusLine 注册上走同步 `execFileSync`
  （超时 5s），而钩子自己的 4s 保护是 `setTimeout`，无法在同步调用阻塞期间触发；
  Claude Code 注册的钩子超时也是 5s，所以子进程的超时只可能在宿主放弃之后才到期
  (hooks/lib/statusline-composite.cjs:60, hooks/gsd-session-init.cjs:186/252)。
  来源：scout-hooks，报告被截断在触发条件处。
- [P2] [已复现] `cli.js` / `install.js` / `uninstall.js` 都不在 lint 范围内
  （`biome check src/ tests/ hooks/`），而它们是安装入口，改动没有静态检查覆盖（`biome check src/ tests/ hooks/`），
...
- R3 [P1] `gsd uninstall` 对错目录报成功，留下全活的安装 → `c843c12`
- R3 [P1] 重装 staging 失败时丢失 runtime/ 状态并留下孤儿目录 → `2abef9b`
