...
describe('server tool handling', () => {
  it('returns structured errors for invalid tool input', async () => {
    ...
    const result = await handleToolCall('state-update', { updates: null, basePath: process.cwd() });
  ...
  it('can resume minimal orchestration through server tool', async () => {
    ...
    try {
      const { handleToolCall } = await import('../src/server.js');
      await handleToolCall('state-init', {
        project: 'orchestrator-server-test',
        phases: [{ name: 'P1', tasks: [{ index: 1, name: 'Task A' }] }],
        basePath: tempDir,
      });

      const result = await handleToolCall('orchestrator-resume', { basePath: tempDir });
  ...
  it('can route executor failure to debugger through server tools', async () => {
    ...
    try {
      ...
      await handleToolCall('state-update', {
        updates: {
          current_task: '1.1',
      ...
      const result = await handleToolCall('orchestrator-handle-executor-result', {
        basePath: tempDir,
        result: {
          task_id: '1.1',
          outcome: 'failed',
  ...
  it('can persist researcher output through server tool and continue orchestration', async () => {
    ...
    try {
      ...
      const result = await handleToolCall('orchestrator-handle-researcher-result', {
        basePath: tempDir,
        result: {
...
describe('MCP tool call chain', () => {
  ...
  it('read after init returns full state', async () => {
    const state = await handleToolCall('state-read', { basePath: tempDir });
  ...
  it('walk task B through full lifecycle', async () => {
    const toRunning = await handleToolCall('state-update', {
      updates: { phases: [{ id: 1, todo: [{ id: '1.2', lifecycle: 'running' }] }] },
      basePath: tempDir,
    });
    assert.equal(toRunning.success, true);

    const toCheckpointed = await handleToolCall('state-update', {
      updates: {
        phases: [{
          id: 1,
    ...
    const toAccepted = await handleToolCall('state-update', {
  ...
  it('phaseComplete succeeds when all tasks accepted', async () => {
    const result = await handleToolCall('phase-complete', {
      phase_id: 1,
      basePath: tempDir,
      verification: {
        lint: { exit_code: 0 },
...
describe('health tool', () => {
  it('returns health status without state', async () => {
    const { handleToolCall } = await import('../src/server.js');
    const fakePath = '/tmp/nonexistent-health-' + Date.now();
    const result = await handleToolCall('health', { basePath: fakePath });
  ...
  it('returns health status with project info when state exists', async () => {
    const healthDir = await mkdtemp(join(tmpdir(), 'health-'));
    try {
      const { handleToolCall } = await import('../src/server.js');
      await handleToolCall('state-init', {
        project: 'health-test',
      ...
      const result = await handleToolCall('health', { basePath: healthDir });
      assert.equal(result.status, 'ok');
...
describe('server dispatch coverage', () => {
  it('dispatches state-read with null args gracefully', async () => {
    const { handleToolCall } = await import('../src/server.js');
    const result = await handleToolCall('state-read', null);
    // Should still work (args defaults to {})
    assert.ok(result);
  });

  it('dispatches orchestrator-resume with null args', async () => {
    const { handleToolCall } = await import('../src/server.js');
    // Will fail due to no .gsd dir, but should not throw
    const result = await handleToolCall('orchestrator-resume', null);
    assert.ok(result);
  });

  it('dispatches orchestrator-handle-executor-result with null args', async () => {
    const { handleToolCall } = await import('../src/server.js');
    const result = await handleToolCall('orchestrator-handle-executor-result', null);
    assert.equal(result.error, true);
    assert.match(result.message, /result must be an object/);
  });

  it('dispatches orchestrator-handle-debugger-result with null args', async () => {
    const { handleToolCall } = await import('../src/server.js');
    const result = await handleToolCall('orchestrator-handle-debugger-result', null);
    assert.equal(result.error, true);
    assert.match(result.message, /result must be an object/);
  });

  it('dispatches orchestrator-handle-researcher-result with null args', async () => {
    const { handleToolCall } = await import('../src/server.js');
    const result = await handleToolCall('orchestrator-handle-researcher-result', null);
    assert.equal(result.error, true);
    assert.match(result.message, /result must be an object/);
  });

  it('dispatches orchestrator-handle-reviewer-result with null args', async () => {
    const { handleToolCall } = await import('../src/server.js');
    const result = await handleToolCall('orchestrator-handle-reviewer-result', null);
    assert.equal(result.error, true);
    assert.match(result.message, /result must be an object/);
  });
  ...
  it('catches thrown errors and wraps them as structured error', async () => {
    const { handleToolCall } = await import('../src/server.js');
    // state-init with invalid args that would cause an internal error
    const result = await handleToolCall('state-init', { project: null, phases: null });
    assert.equal(result.error, true);
  });

  it('dispatches health with null args', async () => {
    const { handleToolCall } = await import('../src/server.js');
    const result = await handleToolCall('health', null);
    assert.equal(result.status, 'ok');
    assert.equal(result.server, 'gsd');
  });
  ...
  it('routes reviewer result through server tool', async () => {
    ...
    try {
      ...
      const result = await handleToolCall('orchestrator-handle-reviewer-result', {
        basePath: reviewerDir,
        result: {
          scope: 'task',
          scope_id: '1.1',
  ...
  it('routes debugger result through server tool', async () => {
    ...
    try {
      ...
      const result = await handleToolCall('orchestrator-handle-debugger-result', {
        basePath: debuggerDir,
        result: {
          task_id: '1.1',
